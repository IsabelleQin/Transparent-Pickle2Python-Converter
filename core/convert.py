...
class Convert(pickle._Unpickler):
    def __init__(self, file, *args, indent=4, **kwargs):
    ...
    def load_appends(self):
        ...
        if isinstance(list_obj, ObjRep):
            ...
            self.source_code.write(f"except:\n{self.indent}for item in items: \n{self.indent}{list_obj}.append(item)\n")
            return
        try:
    ...
    def load_additems(self):
        ...
        if isinstance(set_obj, ObjRep):
            ...
            self.source_code.write(f"else:\n{self.indent}for item in items: {set_obj}.add(item)\n")
            return
...
def convert(file, *, indent=4, fix_imports=True, encoding="ASCII", errors="strict",
          buffers=None):
    return Convert(file, indent=indent, fix_imports=fix_imports, buffers=buffers,
