...
from core.torch.configs import *
from torch._sources import get_source_lines_and_file
from typing import Any, cast
from torch.serialization import MAP_LOCATION, IS_WINDOWS, MAP_SHARED, MAGIC_NUMBER, PROTOCOL_VERSION
from torch.serialization import get_default_mmap_options, get_default_load_endianness, LoadEndianness, SourceChangeWarning

serialization_tls = torch.serialization._SerializationLocal()
open_file_like = torch.serialization._open_file_like
is_zipfile = torch.serialization._is_zipfile
is_path = torch.serialization._is_path
open_zipfile_reader = torch.serialization._open_zipfile_reader
check_seekable = torch.serialization._check_seekable
should_read_directly = torch.serialization._should_read_directly
is_torchscript_zip = torch.serialization._is_torchscript_zip
get_restore_location = torch.serialization._get_restore_location
is_meta_location = torch.serialization._is_meta_location
maybe_decode_ascii = torch.serialization._maybe_decode_ascii
...
def configure(opened_zipfile, map_location):
    ...
    loaded_storages = {}
    is_meta_map_location = is_meta_location(map_location)
...
def persistent_load_zip(saved_id):
    ...
    if key in loaded_storages:
        ...
        typed_storage = load_tensor(
            dtype, nbytes, key, maybe_decode_ascii(location)
        )
